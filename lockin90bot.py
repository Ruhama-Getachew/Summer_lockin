...
async def log_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Send your log below. Use this format:\n\n"
        f"`ID: {member_id}`\n"
        f"`Day: {day}/90`\n"
        "`Plan:`\n"
        "`- Task 1`\n"
...
async def testlog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✅ Test log posted to Daily Logs topic!")
    except Exception as e:
        await update.message.reply_text(f"❌ Failed to post: {e}")

    # Also test leaderboard post to general
    try:
        await context.bot.send_message(
            chat_id=GROUP_ID,
        ...
        await update.message.reply_text("✅ Test leaderboard posted to General topic!")
    except Exception as e:
        await update.message.reply_text(f"❌ Failed to post leaderboard: {e}")
...
async def streak_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    member_id = profile.get("member_id") or "Not registered"
